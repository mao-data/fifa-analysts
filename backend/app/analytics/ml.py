...
def probs(model, x: list[float]) -> tuple[float, float, float]:
    """(home win, draw, away win) regardless of which classes were seen."""
    p = model.predict_proba([x])[0]
    by_class = dict(zip(model.classes_, p))
    return tuple(by_class.get(c, 0.0) for c in CLASSES)  # type: ignore[return-value]
